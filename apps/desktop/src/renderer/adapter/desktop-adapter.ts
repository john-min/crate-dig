...
function mapRunTrack(row: LocalRunTrack, runId: string): TrackAnalysis {
  const trackId = row.track_id ?? (typeof row.id === "string" && row.id ? row.id : "");
  return {
    trackId,
    runId,
    readiness: readinessFromAnalysisEvidence({
...
export class DesktopAdapter implements DesktopRuntimeAdapter {
  readonly runtime = "desktop" as const;
  readonly baseUrl: string;
  private readonly requestFetch: typeof fetch;
  private readonly authSession: DesktopAdapterOptions["getAuthSession"];
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new AdapterError({
          ...
          remediation: "Verify that the desktop sidecar is healthy on 127.0.0.1.",
        });
      }
  ...
  async listTracks(options: ListTracksOptions = {}): Promise<readonly Track[]> {
    const path = options.libraryId
      ? `/libraries/${encodeURIComponent(options.libraryId)}/tracks`
      : "/tracks";
  ...
  async getAnalysisRun(runId: string): Promise<AnalysisRun> {
    return mapRun(await this.request<LocalAnalysisRun>(`/analysis-runs/${encodeURIComponent(runId)}`));
  }
  ...
  async cancelAnalysisRun(runId: string): Promise<AnalysisRun> {
    return mapRun(
      await this.request<LocalAnalysisRun>(`/analysis-runs/${encodeURIComponent(runId)}/cancel`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  async getTrackAnalysis(trackId: string, runId?: string): Promise<TrackAnalysis> {
    ...
    return mapAnalysis(
      await this.request<LocalTrackAnalysis>(`/tracks/${encodeURIComponent(trackId)}/analysis${query}`),
    );
  }

  async getTrackNeighbors(trackId: string, options: NeighborOptions = {}): Promise<readonly Neighbor[]> {
  ...
  async importFolder(input: ImportFolderInput): Promise<ImportResult> {
    const body = await this.request<components["schemas"]["FolderImportResponse"]>("/imports/folder", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
